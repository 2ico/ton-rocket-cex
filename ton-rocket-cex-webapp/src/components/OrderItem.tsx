    ...
    : JSX.Element => 
{
    return (
        <Box>
            ...
            >
                <Box flexBasis={"32px"} sx={{ color: (order.orderAction == OrderAction.Buy ? "rgb(49, 181, 69)" : "#FF4C4C")}}>
                ...
                <ListItemText sx={{flexBasis: "120px"}} primary={order.pair.base_name+"/"+order.pair.quote_name} secondary={order.orderType}/>
                <ListItemText sx={{flexBasis: "80px"}} primary={<React.Fragment>{order.price.toNumber().toPrecision(5)}</React.Fragment>}/>
                <ListItemText sx={{flexBasis: "120px"}} primary={<React.Fragment>{order.amount.toNumber().toPrecision(5)} {order.pair.base_name}</React.Fragment>}/>
            </ListItem>
        </Box>
    )
}
